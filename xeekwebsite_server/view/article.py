...
@bp.route('/edit_demo/<string:id>')
def edit_demo(id):
    article = {
        'content':'#### 文章 啊啊啊'
    }
    return render_template('article/edit_demo.html',article=article)
...
def edit(id):
    ...
    if id != '0' or id == None:
        db=get_db()
        article_tuple = db.execute('SELECT * FROM article_markdown WHERE markdown_id=?',(id,)).fetchone()
